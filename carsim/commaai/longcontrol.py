...
class LongControl(object):
  ...
  def update(self, enabled, v_ego, v_cruise, v_target_lead, a_target, jerk_factor, enableGas):
    ...
    v_cruise_mph = round(v_cruise * (1./1.609344))   # what's displayed in mph on the IC
    v_target = min(v_target_lead, v_cruise_mph * 1.609344 * (1./3.6))
