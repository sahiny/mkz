...
def calc_lookahead_offset(v_ego, angle_steers, d_lookahead, VM, angle_offset):
  ...
  sa = (angle_steers - angle_offset) * (np.pi/180.)
...
def calc_desired_steer_angle(v_ego, y_des, d_lookahead, VM, angle_offset):
  ...
  # pdb.set_trace()
  # print(curvature)
  steer_des = VM.get_steer_from_curvature(curvature, v_ego) * (180./np.pi) + angle_offset
  # print(steer_des)
...
class LatControl(object):
  ...
  def update(self, enabled, v_ego, angle_steers, steer_override, d_poly, angle_offset, VM):
    ...
    # d_lookahead = calc_d_lookahead(1., d_poly)
    # print(d_lookahead)
    ...
    return final_steer, sat_flag, d_lookahead, self.y_des, self.angle_steers_des
